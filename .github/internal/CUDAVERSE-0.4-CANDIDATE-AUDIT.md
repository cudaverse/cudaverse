...
| Develop through short reviewed branches | proven at current checkpoint | `develop/0.4` is `898baa6ce2618141198f869289b1d7d441695c33`; CP-01 through CP-10A and their reviewed evidence are recorded in `CUDAVERSE-0.4-CHECKPOINTS.md` | Merge the completed CP-07B evidence and accepted top-k work before selecting one clean candidate. |
...
| Strengthen resident dense reductions, SVD/PCA, distance, stable top-k/kNN, and k-means | proven at current checkpoint for the current dense surface | CP-03A/B/C/D cover explicit batching, bounded-memory k-means, resident PCA prediction, and native SVD/PCA input; CP-07B completes all dense benchmark cases with base/native/torch parity, exact stable indices, residency, and memory evidence | Evaluate and merge only justified top-k changes, then repeat parity, residency, memory, and deterministic-tie gates on the exact candidate. |
...
| Reproducible smoke/full benchmark with timing, transfer, allocator, VRAM, footprint, and provenance | proven at current checkpoint, pending final-candidate repeat | CP-07A retains the complete public-memory smoke contract; CP-07B retains the complete 12-case/36-backend report from exact clean source `deb7712`, generated summary, run/check logs, exact hashes, numerical validation, memory provenance, and workload-specific interpretation | Repeat the unchanged full contract only after accepted top-k work is merged and one exact final candidate is frozen. |

...
| Audit graph, clustering, embeddings, diffusion, and SingleCellExperiment workflow boundaries | in progress | CP-05A proves the resident PCA-to-diffusion-distance boundary; the capability matrix records intentional CPU/hybrid graph, community, UMAP, t-SNE, diffusion, and object-mapping stages | Re-audit every higher-level export and its documentation on the final source; accelerate no stage without parity, residency, and memory evidence. |

...
| Develop only through reviewed checkpoints | proven at current checkpoint | `develop/native-cuda` remote is `f5dfd7d6cb3228cbb888b3e83d7e7d3d5a337332`; CP-01 through CP-07 and their exact gates are recorded in `.github/internal/CUDAVERSE-0.3-CHECKPOINTS.md` | Freeze feature scope, merge this audit refresh, and use the resulting clean development head as the sole final candidate source. |
...
| Cross-platform packaging, ABI/PTX, SBOM, license, and small artifacts | proven at checkpoint, pending for final | All 11 required checks pass on final CP-07 head `2a028abfa2130e5b671a167fa2c85dc05a2e5414`: R-CMD-check `31461230518`, pkgdown `31461230520`, integration `31461230757`, and native-integrity `31461230525` cover Windows, macOS, Ubuntu, R-devel, artifacts, ABI/PTX, supply chain, CPU contract, and the public pkgdown boundary | Repeat on the exact final merged candidate and retain its run and artifact identifiers. |
| README, NEWS, reference, examples, diagnostics, troubleshooting, and pkgdown alignment | proven at CP-07 head, pending final-candidate repeat | CP-07 head `2a028abfa2130e5b671a167fa2c85dc05a2e5414` adds native-first troubleshooting; a post-benchmark source package passes with 0 errors and 0 warnings, and an isolated local pkgdown build passes the public-boundary checker after maintainer records were moved out of the public site | Re-audit the exact final render and record the two intended article URLs as pre-deployment until Pages is separately authorized. |
| Exact review-ready 0.3 candidate and release decision | pending | CP-06 and CP-07 are merged; checkpoint evidence still cannot substitute for final-candidate evidence | Use the post-audit merged development head, build one exact 0.3.0.9000 tarball, run every final gate on that commit, assemble and check the manifest, and write the release/defer/reduce-scope report. |
